...
M = [m.C zeros(n) m.A'; zeros(n) m.lam*m.Mass -m.B'; m.A -m.B zeros(n)];
...
m.innertol = tols(3);
tic
[z_lgmres_mg,flag,~,iter_lgm_mg] = pcg(M,res_changed,outertol,outermaxit,@(b) P(b,m,mode) );
time_lgm_mg = toc;
res_lgm_mg = norm(M*z_lgmres_mg - res_changed);
if(res_lgm_mg > failtolerance || flag ~=0)
    disp(['LGMRES(MG) failed with flag ', num2str(flag),' and residual ', res_lgm_mg/norm(res_changed)])
else 
    disp(['LGMRES(MG) succeeded in ', num2str(time_lgm_mg+time_ass_mg), 's with ', num2str(iter_lgm_mg), '\',num2str(innerlgm_mg), ...
        ' iterations at rel residual of ', num2str(res_lgm_mg/norm(res_changed))])
end
...
mode = "rapoport_mg";
tic
[z_rap_mg,flag,~,iter_rap_mg] = pcg(M,res_changed,outertol,outermaxit,@(b) P(b,m,mode) );
time_rap_mg = toc;
res_rap_mg = norm(M*z_rap_mg - res_changed);
if(res_rap_mg > failtolerance)
    disp(['Rapoport(MG) failed with flag ', num2str(flag),' and residual ', res_rap_mg/norm(res_changed)])
else 
    disp(['Rapoport(MG) succeeded in ', num2str(time_rap_mg+time_ass_mg), 's with ', num2str(iter_rap_mg), '\',num2str(innerrapo_mg), ...
        ' iterations at rel residual of ', num2str(res_rap_mg/norm(res_changed))])
end

%widlund multigrid as inner
mode = "widlund_mg";
tic
[z_wid_mg,flag,~,iter_wid_mg] = pcg(M,res_changed,outertol,outermaxit,@(b) P(b,m,mode) );
time_wid_mg = toc;
res_wid_mg = norm(M*z_wid_mg - res_changed);
if(res_wid_mg > failtolerance)
    disp(['Widlund(MG) failed with flag ', num2str(flag),' and residual ', res_wid_mg/norm(res_changed)])
else 
    disp(['Widlund(MG) succeeded in ', num2str(time_wid_mg+time_ass_mg), 's with ', num2str(iter_wid_mg), '\',num2str(innerwid_mg), ...
        ' iterations at rel residual of ', num2str(res_wid_mg/norm(res_changed))])
end
